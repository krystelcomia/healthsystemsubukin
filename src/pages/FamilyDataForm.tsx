...
const FamilyDataForm = () => {
  ...
  const totalMembersCount = useMemo(() => {
    return records.reduce((sum, rec) => {
      ...
      return sum + (Number(rec.total_members) || mems.length || 0);
    }, 0);
  }, [records]);

  const uniqueSitiosCount = useMemo(() => {
    const sitios = new Set(records.map(r => r.sitio || "Centro").filter(Boolean));
    return sitios.size;
  }, [records]);
    ...
    <div className="w-full space-y-6">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-2 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold border backdrop-blur-md bg-white/10 text-white border-white/20">
              ...
              Barangay Family Census Directory
            </div>
            <h1 className="text-2xl md:text-3xl font-heading font-extrabold tracking-tight text-white">
              Family Data Records
            </h1>
            <p className="text-sm text-white/80 leading-relaxed">
              Overview of all household profiles and family structures — manage family folders, sitio records, or export master census sheets.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            {!isAdmin && (
              <Button
                ...
                size="sm"
                className="bg-white hover:bg-white/90 text-primary font-bold gap-1.5 text-xs shadow-md border-0"
              >
            ...
            <div className="flex items-center border border-white/20 rounded-xl p-1 bg-white/10 backdrop-blur-md">
              <Button
                variant="ghost"
                size="sm"
                className={`h-8 px-3 text-xs gap-1.5 font-semibold transition-colors ${
                  viewMode === "grid"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 no-print">
        ...
        <Card className="relative overflow-hidden border bg-card shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border-primary/20">
          ...
          <CardContent>
            <div className="text-3xl md:text-4xl font-heading font-extrabold text-foreground tracking-tight">
              {loading ? "..." : totalMembersCount.toLocaleString()}
            </div>
            <div className="mt-2 flex items-center text-xs text-muted-foreground font-medium gap-1">
              <UserPlus className="h-3 w-3 text-primary" />
              Family members indexed
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="relative overflow-hidden border bg-card shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border-primary/20">
          ...
          <CardContent>
            <div className="text-3xl md:text-4xl font-heading font-extrabold text-foreground tracking-tight">
              {loading ? "..." : uniqueSitiosCount.toLocaleString()}
            </div>
            <div className="mt-2 flex items-center text-xs text-muted-foreground font-medium gap-1">
              <MapPin className="h-3 w-3 text-primary" />
              Sitios with family records
            </div>
          </CardContent>
        </Card>
      </div>
