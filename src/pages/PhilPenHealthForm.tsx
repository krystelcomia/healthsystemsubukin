...
const PhilPenHealthForm = () => {
  const [form, setForm] = useState({
    name: "", address: "", age: "", birthdate: "", currentDate: new Date().toISOString().split("T")[0],
    bp: "", height: "", weight: "",
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input type="date" value={form.currentDate} onChange={(e) => handleChange("currentDate", e.target.value)} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="space-y-2">
                <Label>BP</Label>
                <Input value={form.bp} onChange={(e) => handleChange("bp", e.target.value)} placeholder="120/80" />
              </div>
              <div className="space-y-2">
                <Label>Height (cm)</Label>
                <Input value={form.height} onChange={(e) => handleChange("height", e.target.value)} placeholder="cm" />
              </div>
              <div className="space-y-2">
                <Label>Weight (kg)</Label>
                <Input value={form.weight} onChange={(e) => handleChange("weight", e.target.value)} placeholder="kg" />
              </div>
              <div className="space-y-2">
                <Label>BMI</Label>
                <div className="h-10 flex items-center px-3 rounded-md bg-secondary text-secondary-foreground font-semibold">{bmi}</div>
              </div>
            </div>
              ...
              {[
                ...
                <div key={q.key} className="flex items-center justify-between py-2 border-b border-border/50 last:border-0">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(v) => handleChange(q.key, v)}
                  />
                </div>
              ))}
            </div>
